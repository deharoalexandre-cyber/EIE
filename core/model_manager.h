...
class ModelManager {
    ...
    std::map<std::string, bool> expert_trace_;
...
public:
    ...
    void setExpertTrace(const std::string & alias, bool enabled) { expert_trace_[alias] = enabled; }
    ...
    bool load(const std::string& alias, const KvConfig& kv, int gpu_id = 0) {
        ...
        if (expert_trace_.count(alias)) p.ews_trace = expert_trace_.at(alias);
